    ...
    <div className="min-h-screen">
      ...
      <section className="relative pt-24 pb-16 px-4 bg-[#0a1628] overflow-hidden min-h-[90vh] flex items-center">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          {["अ", "க", "অ", "ا", "语", "A", "आ", "ங", "খ", "ب"].map((char, i) => (
            <motion.span
              key={i}
              className="absolute text-white/10 font-heading select-none"
              style={{
                left: `${10 + (i * 9)}%`,
                top: `${20 + ((i % 3) * 25)}%`,
            ...
            >
              ...
              >
                <button
                  ...
                  className="bg-primary text-primary-foreground px-6 py-3 rounded-lg font-medium flex items-center gap-2 hover:bg-primary/90 transition-colors shadow-medium"
                >
                  Explore Building Blocks <ArrowDown size={18} />
                </button>
                <button
                  onClick={() => scrollToSection("quick-start")}
                  className="bg-transparent text-white px-6 py-3 rounded-lg font-medium border-2 border-white/50 hover:border-white hover:bg-white/10 transition-all flex items-center gap-2"
                >
